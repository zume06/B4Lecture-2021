...
def create_filter(f1, f2, samplerate):
    ...
    filter = (np.fft.ifft(fd)).real
    filter = filter.reshape(2, samplerate // 2)
    filter = filter[[1, 0], :]
    filter = filter.reshape(-1)
    filter = filter * np.hamming(samplerate)
    result = np.fft.rfft(filter)
    #plt.plot(filter)
...
def main():
    ...
    draw_spectrogram(ax3, "Spectrogram (Original)", spec_orignal,
    wave_length, samplerate) # スペクトログラムの描画(元データ)
    draw_spectrogram(ax4, "Spectrogram (Filtered)", spec_filtered,
